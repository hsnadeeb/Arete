  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <Text style={[TYPOGRAPHY.bodySm, { color: colors.textTertiary, paddingHorizontal: 16, paddingTop: 14, paddingBottom: 6 }]}>
          All your data in one place. Tap a category to view items, tap X to delete.
        </Text>
        ...
        <View style={[styles.actionRow, { paddingHorizontal: 16, marginBottom: 8 }]}>
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.accentBg }]} onPress={handleExport}>
            <Icon name={LUCIDE_ICONS.download} size={14} color={colors.accent} />
            <Text style={[TYPOGRAPHY.btnSm, { color: colors.accent }]}>Export</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.successBg }]} onPress={handleShare}>
            <Icon name={LUCIDE_ICONS.share2} size={14} color={colors.success} />
            <Text style={[TYPOGRAPHY.btnSm, { color: colors.success }]}>Share</Text>
          </TouchableOpacity>
        </View>
          ...
          <View key={section.key} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <TouchableOpacity style={styles.cardHeader} onPress={() => toggle(section.key)} activeOpacity={0.7}>
              ...
              <View style={[styles.countBadge, { backgroundColor: section.color }]}>
                <Text style={[TYPOGRAPHY.btnSm, { color: '#fff' }]}>{data[section.key] !== undefined ? data[section.key].length : '?'}</Text>
              </View>
              <Icon name={expanded === section.key ? LUCIDE_ICONS.chevronDown : LUCIDE_ICONS.chevronRight} size={14} color={colors.muted} />
            </TouchableOpacity>
            ...
            {expanded === section.key && (
              <View style={[styles.cardBody, { borderTopColor: colors.divider }]}>
                {loading[section.key] ? (
                  ...
                  <Text style={[TYPOGRAPHY.bodySm, { color: colors.muted, textAlign: 'center', paddingVertical: 20, fontWeight: '500' }]}>No entries</Text>
                ) : (
                  data[section.key].map((item: any) => (
                    <View key={item.id} style={[styles.itemRow, { borderBottomColor: colors.divider }]}>
                      ...
                      <TouchableOpacity style={[styles.deleteItemBtn, { backgroundColor: colors.errorBg }]} onPress={() => removeItem(section.key, item.id)}>
                        <Icon name={LUCIDE_ICONS.x} size={12} color={colors.error} />
                      </TouchableOpacity>
                    </View>
                  ))
                )}
...
const styles = StyleSheet.create({
  ...
  title: { ...TYPOGRAPHY.h4, marginLeft: 4, flex: 1 },
  ...
  cardLabel: { ...TYPOGRAPHY.h4, flex: 1 },
  ...
  itemDetail: { ...TYPOGRAPHY.captionSm, marginTop: 1 },
