    ...
    <Card title="At a Glance" titleStyle={{ color: tc.textTertiary }}>
      ...
      {nextPrayer ? (
        <View style={[s.nextPrayerBox, { backgroundColor: tc.accentBg }]}>
          ...
          <Text style={[s.npName, { color: tc.heading }]}>{nextPrayer.name}</Text>
          <Text style={[s.npTime, { color: tc.textSecondary }]}>
            at {t12(nextPrayer.time)} · in{" "}
            <Text style={[s.npCountdown, { color: tc.accent }]}>{nextPrayer.remaining}</Text>
          </Text>
        </View>
      ) : (
        <View style={[s.nextPrayerBox, { backgroundColor: tc.bgSecondary }]}>
          <Text style={[s.npLabel, { color: tc.textTertiary }]}>All prayers completed today</Text>
        ...
        <View style={[s.prayerStrip, { marginBottom: 12 }]}>
          {PRAYER_TIMES_ORDER.filter((p) => p !== "sunrise").map((prayer) => {
            ...
            return (
              <TouchableOpacity
                key={prayer}
                onPress={() => togglePrayer(prayer.charAt(0).toUpperCase() + prayer.slice(1))}
                style={[
                  s.pStripItem,
                  {
              ...
              >
                ...
                <Text style={[s.pStripLabel, { color: done ? '#fff' : tc.textTertiary }]}>
                  {PRAYER_DISPLAY[prayer]}
                </Text>
                <Text style={[s.pStripTime, { color: done ? '#fff' : tc.textSecondary }]}>
                  {t12(time)}
                </Text>
                {qada && <Text style={[s.qadaBadge, { color: tc.warning }]}>Q</Text>}
              </TouchableOpacity>
            );
          })}
        </View>
      ...
      {timingsLoading && (
        <ActivityIndicator size="small" color={tc.accent} style={{ marginTop: 8 }} />
      )}
    </Card>
  );
}
...
function QuickStatsWidget() {
  ...
  const items = [
    { l: "Weight", v: dailyLog?.weight ? `${dailyLog.weight} kg` : "\u2014", c: tc.info },
    { l: "Water", v: dailyLog?.water_ml ? `${(dailyLog.water_ml / 1000).toFixed(1)}L` : "\u2014", c: "#0ea5e9" },
    { l: "Steps", v: dailyLog?.steps?.toLocaleString() || "\u2014", c: tc.warning },
    { l: "Mood", v: dailyLog?.mood ? `${"\u2022".repeat(dailyLog.mood)}${"\u25CB".repeat(5 - dailyLog.mood)}` : "\u2014", c: tc.accent },
  ];
  return (
      ...
      <View style={s.statGrid}>
        {items.map((it) => (
          <View key={it.l} style={s.statTile}>
            ...
            <Text style={[s.statLabel, { color: tc.textTertiary }]}>{it.l}</Text>
          </View>
        ))}
      </View>
    </Card>
      ...
      {rows.map((r) => (
        <View key={r.l} style={s.qlRow}>
          ...
          <View style={s.qlInputRow}>
            <TextInput
              style={[s.qlInput, { backgroundColor: tc.bgSecondary, borderColor: tc.borderLight, color: tc.text }]}
              value={r.v}
              onChangeText={r.s}
            ...
            />
            <TouchableOpacity style={[s.qlBtn, { backgroundColor: tc.accent }]} onPress={r.a}>
        ...
        {moods.map((m) => (
          <TouchableOpacity
            ...
            style={[
              ...
              dailyLog?.mood === m.v && { backgroundColor: tc.accentBg, borderColor: tc.accent },
            ]}
            onPress={() => logMood(m.v)}
          >
            ...
            <Text
              style={[
                ...
                dailyLog?.mood === m.v && { color: tc.accent, fontWeight: "700" },
              ]}
            >
              {m.l}
            </Text>
...
function ExpensesWidget() {
  ...
  return (
    <Card title={`Expenses · $${total.toFixed(2)}`} titleStyle={{ color: tc.textTertiary }}>
      <TouchableOpacity style={[s.addExpBtn, { backgroundColor: tc.success }]} onPress={() => setShow(true)}>
        <Text style={s.addExpBtnText}>➕ New Expense</Text>
      </TouchableOpacity>
      {exps.length > 0 ? (
        <View>
          {exps.slice(0, 5).map((t: any, i: number) => (
            <View key={t.id || i} style={[s.expRow, { borderBottomColor: tc.divider }]}>
              <Text style={[s.expCat, { color: tc.heading }]}>
                {CATEGORY_EMOJIS[t.category]} {t.category}
              </Text>
              <Text style={[s.expDesc, { color: tc.textTertiary }]} numberOfLines={1}>
                {t.description}
              </Text>
              <Text style={[s.expAmt, { color: tc.error }]}>-${t.amount.toFixed(2)}</Text>
            </View>
          ))}
        </View>
      ) : (
        <Text style={[s.expEmpty, { color: tc.textTertiary }]}>No expenses today</Text>
      )}
      <Modal visible={show} transparent animationType="slide">
        <View style={[s.modalOverlay, { backgroundColor: tc.overlay }]}>
          ...
          <View style={[s.modal, { backgroundColor: tc.surface }]}>
            <Text style={[s.modalTitle, { color: tc.heading }]}>💰 Add Expense</Text>
              ...
              {EXPENSE_CATEGORIES.map((c) => (
                <TouchableOpacity
                  ...
                  style={[
                    s.catChip,
                    { backgroundColor: tc.bgSecondary, borderColor: tc.borderLight },
                    cat === c && { backgroundColor: tc.successBg, borderColor: tc.success },
                  ]}
                  onPress={() => setCat(c)}
                >
                  <Text style={s.catIcon}>{CATEGORY_EMOJIS[c]}</Text>
                  <Text style={[s.catLabel, { color: tc.textSecondary }, cat === c && { color: tc.success }]}>
                    {c}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            <Text style={[s.label, { color: tc.textSecondary }]}>Amount ($)</Text>
            <TextInput
              style={[s.modalInput, { backgroundColor: tc.bgSecondary, borderColor: tc.borderLight, color: tc.text }]}
              value={amt}
              onChangeText={setAmt}
              ...
              autoFocus
            />
            <Text style={[s.label, { color: tc.textSecondary }]}>Description</Text>
            <TextInput
              style={[s.modalInput, { backgroundColor: tc.bgSecondary, borderColor: tc.borderLight, color: tc.text }]}
              value={desc}
            ...
            <View style={s.modalActions}>
              ...
              >
                <Text style={[s.cancelBtnText, { color: tc.textTertiary }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[s.saveBtn, { backgroundColor: tc.success }]} onPress={handleAdd}>
  ...
  if (!monthlyStats)
    return (
      <Card title="Monthly Stats" titleStyle={{ color: tc.textTertiary }}>
        <Text style={{ color: tc.textTertiary, textAlign: "center", padding: 8 }}>
          No data yet
        </Text>
      </Card>
    );
  const items = [
    { l: "Avg Weight", v: monthlyStats.avg_weight ? monthlyStats.avg_weight.toFixed(1) : "\u2014", c: tc.info },
    { l: "Avg Water", v: monthlyStats.avg_water ? `${(monthlyStats.avg_water / 1000).toFixed(1)}L` : "\u2014", c: "#0ea5e9" },
    { l: "Avg Steps", v: monthlyStats.avg_steps ? Math.round(monthlyStats.avg_steps).toLocaleString() : "\u2014", c: tc.warning },
    ...
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <View>
          ...
          {editing && (
            <View style={[s.editControls, { backgroundColor: tc.accentBg, borderColor: tc.accent + '66' }]}>
              <TouchableOpacity
                style={[s.editArrow, index === 0 && s.editArrowDisabled]}
                onPress={() => moveUp(index)}
                disabled={index === 0}
              >
                <Text style={[s.editArrowText, index === 0 && s.editArrowTextDisabled]}>▲</Text>
              </TouchableOpacity>
              <Text style={[s.editIndex, { color: tc.accent }]}>{index + 1}</Text>
              <TouchableOpacity
                style={[s.editArrow, index === data.length - 1 && s.editArrowDisabled]}
                onPress={() => moveDown(index)}
                disabled={index === data.length - 1}
              >
                <Text style={[s.editArrowText, index === data.length - 1 && s.editArrowTextDisabled]}>▼</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
  ...
  if (!loaded) {
    return (
      <SafeAreaView style={[s.safe, { backgroundColor: tc.bg }]} edges={["top"]}>
        <View style={[s.topbar, { backgroundColor: tc.surface, borderBottomColor: tc.divider }]}>
        ...
        <View style={s.loadingContainer}>
          ...
          <Text style={[s.loadingText, { color: tc.textTertiary }]}>Loading your dashboard...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <SafeAreaView style={[s.safe, { backgroundColor: tc.bg }]} edges={["top"]}>
        <View style={[s.topbar, { backgroundColor: tc.surface, borderBottomColor: tc.divider }]}>
          ...
          <View style={s.topCenter}>
            ...
            <Text style={[s.dateSmall, { color: tc.textTertiary }]}>{todayStr}</Text>
          </View>
        ...
        <View style={s.emptyContainer}>
          ...
          <Text style={[s.emptyTitle, { color: tc.heading }]}>Welcome to your Dashboard</Text>
          <Text style={[s.emptySubtitle, { color: tc.textTertiary }]}>
            Tap refresh to load your data and start tracking.
          </Text>
          <TouchableOpacity style={[s.emptyRefreshBtn, { backgroundColor: tc.accent }]} onPress={refresh}>
      ...
      <View style={[s.topbar, { backgroundColor: tc.surface, borderBottomColor: tc.divider }]}>
        ...
        <View style={s.topCenter}>
          {!editing ? (
            ...
            <Text style={[s.editHint, { color: tc.accent }]}>⬆️⬇️ Use arrows to reorder</Text>
          )}
        </View>
        {!editing ? (
          <>
            ...
            <View style={[s.streak, { backgroundColor: tc.warningBg }]}>
              <Text style={[s.streakText, { color: tc.warning }]}>🔥 {streak}</Text>
            </View>
          </>
        ) : (
          <TouchableOpacity onPress={exitEdit} style={[s.doneBtn, { backgroundColor: tc.accent }]}>
...
const s = StyleSheet.create({
  ...
  editControls: {
    ...
    gap: 16, paddingVertical: 8, marginBottom: 12,
